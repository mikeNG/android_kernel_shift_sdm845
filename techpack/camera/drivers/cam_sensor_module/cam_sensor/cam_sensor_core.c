	...
	struct device_attribute *attr, char *buf)
{
	...
	if (g_io_master_info == NULL)
		return snprintf(buf, 50,
				"g_io_master_info == NULL, ERROR!\n");

	CAM_DBG(CAM_SENSOR, "g_io_master_info: sid = 0x%x, cid = 0x%x\n",
			g_io_master_info->cci_client->sid,
			g_io_master_info->cci_client->cid);
	...
	CAM_DBG(CAM_SENSOR, "chipid = 0x%x, chipid1 = 0x%x, chipid2 = 0x%x\n",
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (!sysfs_create_file_flag) {
		...
		rc = sysfs_create_file(&s_ctrl->pdev->dev.kobj,
				&dev_attr_camera_dpc.attr);
		if (rc < 0)
			CAM_ERR(CAM_SENSOR,
					"Failed to create vendor info sysfs");
	}
#endif
